...
function groupBy<T extends Record<string, any>>(entries: T[],
    key: keyof T): Record<string, T[]> {
...
export default function Table({identifiers, values}: TableProps) {
  ...
  const yearlyEntries = entries.filter(
      (entry) => entry.costType === 'jährlich');
  ...
  return (
		<div className="h-full w-full">
			<div className="pb-6">
				<SummaryCard totalYearlyCost={formatCost(totalYearlyCost)} totalOnceCost={formatCost(totalOnceCost)} />
			</div>
			<div className="flex flex-row items-start justify-center lg:flex-col gap-16 lg:gap-6 rounded-xl w-full">
				...
				{yearlyEntries.length > 0 && (
					<div id="tableYearly" className="flex flex-col bg-gray-100 py-8 px-4 justify-start rounded-xl w-1/2 lg:w-full">
						<div className="font-semibold text-2xl text-center sm:text-start mb-2 pb-2 border-b border-black">{translate('table.yearlyLabel')}</div>
				...
				{onceEntries.length > 0 && (
					<div id="tableOnce" className="flex flex-col bg-gray-100 py-8 px-4 justify-start  rounded-xl w-1/2 lg:w-full">
						<div className="font-semibold text-2xl text-center sm:text-start mb-2 pb-2 border-b border-black">{translate('table.oneTimeLabel')}</div>
