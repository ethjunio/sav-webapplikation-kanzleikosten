			...
			tooltip: {
				callbacks: {
					label: function (context) {
						...
						return `${type === 'statistics' ? translate('stackedBarPlotCost.mean') : translate('stackedBarPlotCost.model')}: ${formattedValue} CHF`;
					},
				},
			},
			datalabels: {
				...
				align: function (context) {
					...
					if (currentValue === firstValue || context.dataIndex === 0 || firstValue - currentValue < factor * firstValue) {
						return 'start';
					} else {
						return 'end';
					}
		...
		<div className="w-full h-full flex items-start flex-col justify-center sm:gap-5">
			...
			<div id="switchButtons" className="flex self-end md:self-start">
				<button className={`px-4 py-2 mr-2 ${currentData === 'yearly' ? 'bg-primaryFade text-white' : 'bg-gray-200'} rounded-lg`} onClick={() => setCurrentData('yearly')}>
					{translate('stackedBarPlotCost.yearlyCosts')}
				</button>
				<button
				...
				>
					{translate('stackedBarPlotCost.oneTimeCosts')}
				</button>
			</div>
