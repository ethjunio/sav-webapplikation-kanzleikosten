...
const ProgressBarItem: React.FC<ProgressBarItemProps> = ({ icon, text, progressKey, currentProgress }) => {
  ...
  const variantClasses =
    isInProgress || isDone ? 'bg-primary text-white border-primary' : 'bg-gray-200 text-gray-400 border-gray-400';
  ...
  return (
    <div className={`flex flex-row items-center ${progressKey === 'finances' ? 'w-fit' : 'w-full'}`}>
      ...
      <div
        className={`flex flex-col items-center justify-center gap-1 ${isInProgress ? 'scale-125' : ''} ${transitionClasses}`}
      >
        <div
          className={`flex w-fit h-fit items-center justify-center p-3 border rounded-full ${variantClasses} ${transitionClasses}`}
        >
          {icon}
        </div>
        <div
          className={`text-xs ${isInProgress || isDone ? 'text-primary font-semibold' : 'text-gray-400'} ${transitionClasses}`}
        >
          {text}
        </div>
      </div>
