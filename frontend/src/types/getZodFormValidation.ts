  ...
  return z.object({
    ...
    locationNumber: z
      ...
      .refine((val) => !isNaN(val), {
        message: errorMessages.locationNumberInvalid,
      }) // Ensure it's a valid number
      .refine((val) => val > 0, {
        message: errorMessages.locationNumberGreaterThanZero,
      }) // Ensure it's positive
      .refine((val) => Number.isInteger(val), {
        message: errorMessages.locationNumberInteger,
...
export const getZodFormValidationProcess = (language: string) => {
  ...
  return z.object({
    processLeadingPersonnel: z
      ...
      .transform((val) => parseFloat(val)) // Convert the string to a number
      .refine((val) => !isNaN(val), {
        message: errorMessages.processPersonnelPercentageInvalid,
      }) // Ensure it's a valid number
      .refine((val) => val >= 0 && val <= 100, {
        message: errorMessages.processPersonnelPercentageRange,
    ...
    employeesCount: z
      ...
      .transform((val) => parseFloat(val)) // Convert the string to a number
      .refine((val) => !isNaN(val), {
        message: errorMessages.employeeCountInvalid,
      }) // Ensure it's a valid number
      .refine((val) => val > 0, {
        message: errorMessages.employeeCountGreaterThanZero,
      }) // Ensure it's positive
      .refine((val) => Number.isInteger(val), {
        message: errorMessages.employeeCountInteger,
      }), // Ensure it's a full integer
    partnersCount: z
      .string()
      ...
      .transform((val) => parseFloat(val)) // Convert the string to a number
      .refine((val) => !isNaN(val), {
        message: errorMessages.partnerCountInvalid,
      }) // Ensure it's a valid number
      .refine((val) => val > 0, {
        message: errorMessages.partnerCountGreaterThanZero,
      }) // Ensure it's positive
      .refine((val) => Number.isInteger(val), {
        message: errorMessages.partnerCountInteger,
      }), // Ensure it's a full integer
  });
};
...
export const getZodFormValidationFinances = (language: string) => {
  ...
  return z.object({
    revenuePerYear: z
      ...
      .transform((val) => parseFloat(val)) // Convert the string to a number
      .refine((val) => !isNaN(val), {
        message: errorMessages.revenuePerYearInvalid,
      }) // Ensure it's a valid number
      .refine((val) => val > 0, {
        message: errorMessages.revenuePerYearGreaterThanZero,
      }), // Ensure it's positive
    operatingCostsPerYear: z
      ...
      .transform((val) => parseFloat(val)) // Convert the string to a number
      .refine((val) => !isNaN(val), {
        message: errorMessages.operatingCostsPerYearInvalid,
      }) // Ensure it's a valid number
      .refine((val) => val > 0, {
        message: errorMessages.operatingCostsPerYearGreaterThanZero,
      }), // Ensure it's positive
  });
};
